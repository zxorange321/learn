...
from sklearn.preprocessing import StandardScaler## 设置字符集，防止中文乱码
mpl.rcParams['font.sans-serif']=[u'simHei']
mpl.rcParams['axes.unicode_minus']=False
## 拦截异常
warnings.filterwarnings(action = 'ignore', category=ConvergenceWarning)## 数据读取并处理异常数据
...
datas.head(5) ## 显示一下# 1. 数据提取以及数据分隔
## 提取
X = datas[names[1:10]]
Y = datas[names[10]]

## 分隔
X_train,X_test,Y_train,Y_test = train_test_split(X,Y,test_size=0.1,random_state=0)# 2. 数据格式化(归一化)
ss = StandardScaler()
X_train = ss.fit_transform(X_train) ## 训练模型及归一化数据# 3. 模型构建及训练
...
re=lr.fit(X_train, Y_train)# 4. 模型效果获取
r = re.score(X_train, Y_train)
print ("R值（准确率）：", r)
print ("稀疏化特征比率：%.2f%%" % (np.mean(lr.coef_.ravel() == 0) * 100))
print ("参数：",re.coef_)
print ("截距：",re.intercept_)
print(re.predict_proba(X_test))# # 5. 模型相关信息保存
# ## 引入包
# from sklearn.externals import joblib
# ## 要求文件夹必须存在
# joblib.dump(ss, "datas/logistic/ss.model") ## 将标准化模型保存
# # joblib.dump(lr, "datas/models/logistic/lr.model") ## 将模型保存# # 模型加载
# ## 引入包
# from sklearn.externals import joblib
# oss = joblib.load("models/logistic/ss.model")
# olr = joblib.load("models/logistic/lr.model")# 数据预测
## a. 预测数据格式化(归一化)
X_test = ss.transform(X_test) # 使用模型进行归一化操作
## b. 结果数据预测
Y_predict = re.predict(X_test)## c. 图表展示
